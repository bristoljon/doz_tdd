...
describe("Calculator UI tests", function() {
  ...
  it("Calls calc.equals on equals click", function() {
    ...
    return expect(dom.output.innerHTML).toBe('230');
  });
  it("calls 'reset' method and clears display on 'C' key press", function() {
    ...
    return expect(output.innerHTML).toBe('');
  });
  ...
  xit("Converts equation from decimal to dozenal on 'mode' click", function() {
    var click_event;
    click_event = new MouseEvent('click');
  ...
  return xit("Converts equation from dozenal to decimal on 'mode' click", function() {
    dom.mode.dispatchEvent(click_event);
    return expect(output.innerHTML).toBe('10');
  });
});

xdescribe("Calculator Logic tests", function() {
  beforeEach(function() {
    calc = new Calculator;
    return calc.init();
  });
  describe("Dozenal / Decimal conversion", function() {
    it("Converts XL to ab strings and vice versa", function() {
      ...
      return expect(calc.transform("abba")).toBe("XLXL");
    });
